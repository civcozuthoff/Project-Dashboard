...
def parse_project_xml(file_like):
    # Returns: summary(dict), open_actions(DataFrame), tasks(DataFrame), owner_map(DataFrame)
    ...
    for r in root.findall("msproj:Resources/msproj:Resource", ns):
        ...
        if uid is not None and nm is not None:
    ...
    for a in root.findall("msproj:Assignments/msproj:Assignment", ns):
        ...
        if r_uid in res_uid_to_name and t_uid:
            pairs.append({"TaskUID": t_uid, "Owner": res_uid_to_name[r_uid]})
    df_pairs = pd.DataFrame(pairs)

    # Fallback to inline ResourceNames
...
st.dataframe(
    df_summary[["ProjectName","OverallPercentComplete","OpenActions","CriticalOpenActions","LateOpenActions","PlannedFinish","BullFinish","BearFinish","Status"]]
)
...
if not df_open_f.empty:
    if not df_owner_pairs_all.empty:
        ...
        tmp["Owner"] = tmp["Owner"].fillna("Unassigned")
    owner_counts = tmp.groupby("Owner")["TaskUID"].count().sort_values(ascending=False).reset_index(name="OpenActions")
    with colB:
        fig, ax = plt.subplots(figsize=(4,3))
        ax.bar(owner_counts["Owner"], owner_counts["OpenActions"])
        ax.set_title("Open Actions by Owner", fontsize=12)
...
if not df_open_f.empty:
    ...
    with colC:
        if not age_tbl.empty:
            ...
            ax2.set_title("Aging by Project", fontsize=12)
            ax2.set_xlabel("Project"); ax2.set_ylabel("Tasks")
            ax2.legend(title="Days Late", fontsize=8)
...
# Top 5 late tasks by duration (across filtered projects)
if not df_open_f.empty:
    late = df_open_f.copy()
    ...
    if not late.empty:
        ...
        # Join owners for better display when available
...
# Next 3 upcoming actions for each project
if not df_open_f.empty:
...
# Top Open Actions table
st.subheader("Top Open Actions (filtered)")
cols_show = ["Project","TaskName","PercentComplete","Critical","Start","Finish","Notes"]
st.dataframe(
    df_open_f.sort_values(by=["Critical","Finish","TaskName"], ascending=[False,True,True])[cols_show].head(50)
)

# ---- Project detail tabs with Critical Path listing ----
    ...
    if projs:
        ...
        for tab, prj in zip(tabs, projs):
            with tab:
                ...
                crit = tdf[tdf["Critical"] == True].copy()
                crit = crit[~crit["IsSummary"]]
                ...
                ms = tdf[tdf["IsMilestone"] == True].sort_values("Finish")
                st.markdown("**Milestones:**")
                st.dataframe(ms[["TaskUID","TaskName","Finish","Notes"]].head(50))

                cta, ctb, ctc = st.columns(3)
                with cta:
                    odf = df_open_f[df_open_f["Project"] == prj].dropna(subset=["Finish"])
                    if not odf.empty:
                        bd = odf.assign(FinishDate=lambda d: pd.to_datetime(d["Finish"]).dt.date)\
                                .groupby("FinishDate").size().reset_index(name="OpenTasks")\
                                .sort_values("FinishDate")
                        small_line(bd, "FinishDate", "OpenTasks", "Burndown")
                with ctb:
                    st.write("**Bull/Bear Finish**")
                ...
                with ctc:
                    ...
                    crit_n = int((df_open_f[(df_open_f['Project']==prj) & (df_open_f['Critical']==True)]).shape[0]) if not df_open_f.empty else 0
